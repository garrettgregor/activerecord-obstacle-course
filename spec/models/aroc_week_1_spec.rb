...
# You can comment out the Ruby example after your AR is working.
  ...
  it '2. finds order id of smallest order' do
    ...
    # order_id = Order.order(:amount).select(:id).limit(1).pluck(:id).first
    order_id = Order.order(:amount).select(:id).first.id
    # ------------------------------------------------------------
  ...
  it '5. finds multiple items by id' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    items = Item.select(:id).where(id: ids_to_find)
    # ------------------------------------------------------------
  ...
  it '6. finds multiple orders by id' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    orders = Order.select(:id).where(id: ids_to_find)
    # ------------------------------------------------------------
