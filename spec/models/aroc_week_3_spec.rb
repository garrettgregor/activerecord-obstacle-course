...
# ----------------------
  ...
  it '18. returns the names of items for a users order' do
    ...
    items_for_user_3_third_order = @user_3.orders.limit(3).last.items.pluck(:name)
    # items_for_user_3_third_order = Order.joins(:items).where(user_id: @user_3.id).limit(3).first.items.pluck(:name)
  ...
  it '20. returns the average amount for all orders for one user' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    average = Order.joins(:user).where(user_id: @user_3.id).average(:amount)
    # ------------------------------------------------------------
